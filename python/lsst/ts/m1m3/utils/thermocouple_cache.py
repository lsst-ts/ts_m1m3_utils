...
class ThermocoupleCache:
    ...
    def add_temperatures(
        ...
        match = self.SENSOR_NAME_RE.match(sensor_name)
        if match is None:
            raise ValueError(f"Unexpected sensorName for index {sal_index}: {sensor_name!r}")
        chunk_index = int(match[1]) - 1

        updated = False
        for channel, temperature in enumerate(temperatures[: self.CHANNELS_PER_MESSAGE]):
            thermocouple = find_thermocouple(scanner, chunk_index * self.CHANNELS_PER_MESSAGE + channel)
