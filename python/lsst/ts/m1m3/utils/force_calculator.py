...
class ForceCalculator:
    ...
    def load_config(self, config_dir: str | pathlib.Path) -> None:
        ...
        for axes in ["XZ", "YZ", "XY"]:
            self.velocity_tables.append(
                ForceTable().load(
                    tables_path / self.fas[f"Velocity{axes}TablePath"],
                    FATABLE_ZFA,
                ),
            )
    ...
    def save(
        ...
        for i, axes in enumerate(["XZ", "YZ", "XY"]):
            self.velocity_tables[i + 3].save(
                out_dir / self.fas[f"Velocity{axes}TablePath"],
                comments,
                reset_comments,
            )
    ...
    def set_acceleration_and_velocity(self, sets: pd.DataFrame) -> None:
        ...
        for row in FATable:
            ...
            def do_update(a: str, idx: int, coeff: pd.Series) -> None:
                ...
                for tab in range(3):
                    self.acceleration_tables[tab].data.loc[idx, a] += coeff[tab + 6]
    ...
    def update_acceleration_and_velocity(self, updates: pd.DataFrame) -> None:
        ...
        updates : pd.DataFrame
            New acceleration (3) and velocity (3+3) coefficients matrices
        """
        for row in FATable:
